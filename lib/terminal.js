...
let spawns_tty = function(log, options, nocp, ...args) {
    let tty;
    ...
    switch (process.platform) {
        ...
        case 'linux':
        case 'win32':
            let pargs = [this.has_a_tty.option];
            if (nocp !== true) {
                pargs = [...pargs, this.has_a_tty.call, this.has_a_tty.script];
            }
            tty = spawn(this.has_a_tty.shell, [...pargs, ...args], options);
            break;
        case 'darwin':
            tty = spawn(

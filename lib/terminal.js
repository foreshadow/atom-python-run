...
class SpawnWrapper extends Base {
    ...
    spawnWin32(options, ...args) {
        let opts = this.object.option;
        return spawn(
            this.object.shell,
            [ ...this.object.option,
                this.object.call,
    ...
    spawnDarwin(options, ...args) {
        return spawn(
            this.object.shell,
            [ this.object.option,
                this.object.command(...args)
            ],
            options
        );
    }

    spawnWrapper(options, ...args) {
        switch (process.platform) {
            case 'linux':
            case 'win32':
                return this.spawnWin32(options, ...args);
            case 'darwin':
                return this.spawnDarwin(options, ...args);
            default:
                return null;
        }
    }
    ...
    tty(object) {
        ...
        let options = this.optionWrapper(object.options);
        let tty = this.spawnWrapper(options, ...args);
        if (object.log) {
            let log = new Log({
                'tty': tty,
                'options': options,
...
class Type {
    constructor () {
        this.has_a_tty = new Shell().object;
        this.spawn = new SpawnWrapper(this.has_a_tty);
    }
}
